...
func start_minigame():
	...
	_reset_game()
	_state = STATE.READY  # ⭐ Start dengan READY state
	_ready_timer = 1.0  # ⭐ 1 detik delay
	set_process(true)
	...
	print("🎮 Minigame started! Preparing...")
	print("🎯 Hold ATTACK to move bar, keep it over the fish!")
...
func _success():
	...
	print("🐟 Caught: %s | %.1fcm | %.2fkg | $%d" % [fish.fish_name, _fish_size, _fish_weight, fish_value])
